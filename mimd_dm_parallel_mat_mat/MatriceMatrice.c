...
double* generate_random_matrix(int, double, double);
...
int main(int argc, char** argv) {
	...
	/*		
		...
		periods: Array delle perioditcità della griglia di processori
	...
	*/
	...
	if(!mpi_rank) {
		srand(time(NULL));
		matrix_a = generate_random_matrix(matrix_order, 0.0, 5.0);
		matrix_b = generate_random_matrix(matrix_order, 0.0, 5.0);
	...
	if(matrix) {
		for(int i = 0; i < matrix_order; i++)
			for(int j = 0; j < matrix_order; j++)
				matrix[i*matrix_order+j] = (((double)rand()*(upper-lower))/(double)RAND_MAX+lower);
	}
    return matrix;
...
/*
	...
	@params:
		...
		int mpi_size: Numero di processori impiegati
		int periods: Periodicità della griglia
